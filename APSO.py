...
def create_folder_path(g, b, result): # function for create folder path
    ...
    day = now.strftime("%d")
    year = now.strftime("%Y")
    formatted_date = f"{day}-{month}-{year}-gamma={g}-beta={b}-result={result:.2f}-APSO"
...
class Particle: #function khai báo giá trị đầu tiên của quần thể
    def __init__(self, bounds , n_particles, n_dimensions, position_file_path):
        ...
        self.position = self.read_position_file(position_file_path, n_particles, n_dimensions) 
       
        self.velocity = np.random.uniform(-1, 1, size=(n_particles, n_dimensions)) #khởi tạo ramdom vận tốc ban đầu
    ...
    def read_position_file(self, file_path, n_particles, n_dimensions): #đọc fie .txt cho vị trí ban đầu
        positions = np.zeros((n_particles, n_dimensions))
        with open(file_path, 'r') as file:
            lines = file.readlines()
            values = [float(line.strip()) for line in lines]  
            if len(values) != n_particles * n_dimensions:
                raise ValueError(f"The file does not contain the correct number of values for {n_particles} particles and {n_dimensions} dimensions.")
            values = np.array(values).reshape(n_dimensions, n_particles).T #sắp xếp các giá trị theo array[n_dimensions, n_particles]
            positions = values
        return positions
...
def update_velocity(particle, global_best_position , n_iter): # Cập nhật vận tốc và vị trí của các hạt
    ...
    Epsilon = np.random.rand() #ramdom trong khoảng (0-1)
    Gamma = 0.5
    Ampha = Gamma**n_iter
    Beta = 0.5
...
def update_position(particle, bounds): # Cập nhật vị trí mới theo vị trí tốt nhất
    new_position = particle.position + particle.velocity
    new_position = np.clip(new_position, bounds[:, 0], bounds[:, 1])

    return new_position
    
def pso(bounds, n_particles, n_dimensions ,n_requirement, n_iter, df): #Function tính toán thuật toán
    #Khởi tạo vị trí và vận tốc ban đầu cho các hạt
    particles = [Particle(bounds, n_particles, n_dimensions,position_file_path) for _ in range(1)] 
    #khai bao hàm
    ...
    for i in range(n_iter): #số lần lặp
        ...
        for particle in particles:
            particle.A_fitness,param_result = calFitness(particle.position,n_particles,n_requirement) #lấy giá trị chuỗi fitness 
            ...
            if particle.best_fitness > global_best_fitness: #nếu best fitness mới > best fitness của toàn quần thể thì sẽ thay đổi
                global_best_fitness = particle.best_fitness
                global_best_position = particle.best_position.copy()
        
        for particle in particles: #Gọi cập nhập lại vị trí và vận tốc mới cho vòng lặp tiếp theo
...
if __name__ == "__main__":
    ...
    u = 1e-6
    p = 1e-12
    position_file_path = 'APSO_first_params.txt'
    
    #tên các cột của excel
    ...
    positions,Best_fitness,df = pso(bounds, n_particles, n_dimensions,n_requirement,n_iter,df) #gọi thuật toán 
    print('Done!')
    print(df)
