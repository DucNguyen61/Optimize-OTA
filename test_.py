...
def calFitness(sol,particles, n_requirement):
    ...
    for i in range(particles):
        ...
        if Cond[i] == 0:
            fitness[i] = -1
        elif ( (PM[i] < 60.0) | (Gain[i] < 50.0) | (GBW[i] < 50) | (Power[i] > 250.0) | (CMRR[i] < 50) | (PSRR_p[i] < 50) | (PSRR_n[i] < 50) | (SR[i] < 50) ):
            fitness[i] = 0
        else: 
...
class Particle:
    def __init__(self, n_bits, n_particles, n_dimensions):
...
def pso(bounds, n_particles, n_bits, n_dimensions ,n_requirement, n_iter, df):
    ...
    particles = [Particle(n_bits, n_particles, n_dimensions) for _ in range(1)]
